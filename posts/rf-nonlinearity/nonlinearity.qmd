...
$$\text{NF} = \frac{SNR_\text{in}}{SNR_\text{out}}$$

or Noise figure in dB as
$$\text{NF}|_\text{dB} = 10\log(\frac{SNR_\text{in}}{SNR_\text{out}})$$
...
To define noise in a cascade of stages, we can use Friis's equation to determine total Noise Figure as following :

$$NF_{\text{total}} = {NF}_1 + \frac{NF_2 - 1}{A_{P1}}$$
