...
Keywords: architecture, overfit, fine-tunning, validation set, test set, weights, parameters, CNN

# Introduction 
Deep learning is common in these day technology. It has been applied in many field such as Computer Vision (CV),
Natural language processing (NLP) as well as in Medicine. The first idea of neurons network came from Pitts and McCulloch. **Mark I Perceptron** The first artificial neuron network was built using the concept from Pitts and McCulloch, but the Perceptron was a single layer neuron network and it couldn't learn some simple functions (such as XOR). 
